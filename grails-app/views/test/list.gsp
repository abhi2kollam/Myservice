			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="value" title="${message(code: 'test.value.label', default: 'Value')}" />
				...
				<g:each in="${testInstanceList}" status="i" var="testInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${testInstance.id}">${fieldValue(bean: testInstance, field: "description")}</g:link></td>
					
						<td>${fieldValue(bean: testInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: testInstance, field: "value")}</td>
